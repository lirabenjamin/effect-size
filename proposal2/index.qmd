...
## Study 1
We survey the meta-analysis literature in psychology in the past 5 years. From that search, we find over 5K published meta-analyses, of which 3.5K have accessible pdfs. From these we get an idea of how are meta-analysts dealing with publication bias. We find that X% of studies mention publication bias, and Y% of studies use some method to correct for publication bias. We also find that Z% of studies report unpublished effect sizes or the size of the difference between published and unpublished studies. Most common methods of adjustment are x (x%), y (y%), and z (z%). From this we learn that meta-analysts don't always analyze gray literature, and tend to not deal with publication bias or deal with it in weak ways.

## Study 2
We survey pre-registered meta-analyses with existing data posted online. When publication status is reported, we compute the empirical distributions of effect size conditional on publication status. We estimate the file drawer effect size from this. When gray literature is not examined, I apply ROBMA to each dataset, and then compare the original and corrected effect size. We estiamte a credible correction factor across the discipline from this. We als estimate a conditional correction factor based on characteristics of the meta-analysis.

Why ROBMA, since we are esimtating this across psychology, we need a method that is robust to across-study heterogeneity, which RoBMA allows @maierRobustBayesianMetaAnalysis2023.

## Study 3
A replication of study 2, but with a different sample of meta-analyses. we email meta analysis authors from study 1 from the past 5 years, and obtain their data. We estimate the file drawer effect and the correction factor as before from this.

## Study 4
We conduct a survey of researcher expectations. We ask researchers to estimate a correction factor.
